...
def encode_Y(input_array : np.array, clnum : int) -> np.array:
    ret = np.eye(clnum)
    ret = ret[input_array]

    return ret

#print(encode_Y(np.array([0,2,0,3]), 4))
...
def eval_classification(i_items : list, i_prob : np.array) -> str:
    return i_items[np.argmax(i_prob)]
...
def array_multi(i_array : np.array) -> int:
    return np.prod(i_array)
...
def array_multi_2d(i_array : np.array) -> np.array:
    return np.prod(i_array, axis=1)
...
def add_border(i_array : np.array) -> np.array:
    return np.pad(i_array, pad_width=1, mode='constant',constant_values=0)
...
def list_days(d1 : str, d2 : str) -> np.array:
    ds = np.datetime64(d1 + "-01")
    de = np.datetime64(d2 + "-01")
    ret = np.array(np.arange(ds, de),np.datetime64)
