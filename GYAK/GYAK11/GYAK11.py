...
Original file is located at
    https://colab.research.google.com/drive/1Byh2nOWGogEZXKfpYCdKWxTxn_GzaWKV
...
#pic = np.zeros((5,5))
print("bemeneti kép:")
pic = np.random.randint(2, size=(5,5))
print(pic)
...
print("kimeneti kép shape:")
op_height = (p_height - k_height) / (1)+1
op_width =  (p_width - k_width) / (1)+1
...
zeros = np.zeros((op_height,op_width))
print(zeros)
...
kernel_size=3
kernel_num=1
for h in range(p_height-kernel_size+1):
  for w in range(p_width-kernel_size+1):
    patch =  pic[h:(h+kernel_size), w:(w+kernel_size)]#indexelj ki egy kernelnyi mátrixot a bemeneti képből
    patches.append((patch, h, w))

#print(patches)

convolution_output = zeros
for patch, h, w in patches:
  convolution_output[h,w] = np.sum(patch*kernel, axis=(0,1))      #Végezd el a konvolúciós lépést

print("output:\n {}".format(convolution_output))
